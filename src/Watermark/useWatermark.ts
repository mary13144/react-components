...
const getMergedOptions = (o: Partial<WatermarkOptions>) => {
	...
	const mergedOptions: WatermarkOptions = {
		...options,
...
const getCanvasDate = async (
	...
	function drawText(): Promise<{
		width: number;
		height: number;
		base64Url: string;
	}> {
		const { fontSize, color, fontWeight, fontFamily } = fontStyle;
		const realFontSize = toNumber(fontSize, 16);
...
export default function useWatermark(params: WatermarkOptions) {
	...
	function drawWatermark() {
		...
		getCanvasDate(mergedOptions).then((res) => {
			...
			const wmStyle = `
                ...
                background-size:${gap?.[0] + res.width}px ${gap?.[1] + res.height}px;
                background-repeat:repeat;
                background-image:url(${res.base64Url})`;
